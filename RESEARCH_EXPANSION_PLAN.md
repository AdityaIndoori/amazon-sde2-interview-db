...
2. Input `data/unconfirmed/*.json`: `{slice, reports:[{id,url,title,role,location,date,dateBasis,stageEvidence,questions:[{round:null,key,question,topic,evidence,roundUncertainty}]}]}`. Same optional original metadata/aliases as strict accounts. Validate actual calendar dates and source level, explicit null round, required uncertainty and provenance.
