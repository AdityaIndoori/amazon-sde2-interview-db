...
**All** (`?view=all`) displays both collections together: 621 entries, 670 occurrences, and 143 distinct reports without filters. Rows keep their original round status and frequency; shared source reports count once. Numbered round filters exclude unconfirmed rows. Bookmarks/practiced marks remain shared with each row's original collection. Selection CSV includes both statuses; full JSON/SQLite downloads remain separately labeled for each collection. All is unavailable if the supplemental dataset cannot load.
