...
The **Round verified** collection contains 590 rows, 639 occurrences and 137 reports. **Round unconfirmed** contains 31 questions from 7 accounts whose SDE II final-loop stage is supported but whose question ordinals are missing. One account contributes different questions to both collections; their unique combined report count is 143, not 144. Supplemental questions never change verified-round frequencies. Explicit collection URLs use `?view=strict` or `?view=unconfirmed`.

**All** is the default collection (also available with `?view=all`): 621 entries, 670 occurrences, and 143 distinct reports without filters. The Round filter offers rounds 1–4 and **Round unconfirmed** (`round=unconfirmed`), which can be selected alone or alongside numbered rounds. Round choices apply in every collection; incompatible collection/round selections yield no matches. Reset clears all filters within the current collection. Rows retain their frequencies and study marks, and shared reports count once. Shared URLs and newly saved views explicitly record the collection. All falls back to Round verified if the supplemental dataset cannot load. CSV follows the selection; full JSON/SQLite downloads remain separate.
...
The main page is preparation-only: search, collection/round/category/progress filters, and the question table. Topic, country, year, date basis, evidence quality, and minimum frequency live under **More filters**, with an active-filter count. Saved views and sharing are collapsed until needed. Methodology, campaign filters, discovery queue, run/change histories, historical exclusions, filter guidance, and full database downloads are on [Research & provenance](research.html). Research-only requests and rendering no longer block question loading.
...
- Export the filtered selection as CSV; download full verified/unconfirmed JSON and SQLite from Research & provenance.
